...
(defun engine_v4.04 (no-variables domain var-type abs-interval? start rulelist heurrulelist mode rnd? read-write-engine-state?)
		;;;;;;;; setup searchspace
									...
									(declare (type list i-vars-candidates))
										(setf i-vars-candidates (loop for i fixnum in (get-varidx-for-any-rule (get-rule heurrulevector heurruleid) (get-index searchindex))
																	collect  (if (= i (get-index searchindex))
													        				    (list (-  (candidate searchspace i candidate-idx) (aref solution i 1)) (candidate searchspace i candidate-idx))
