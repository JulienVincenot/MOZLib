...
(defmacro a (candidate) `(if (listp (car ,candidate))
	(mapcar 'second ,candidate)
	(second ,candidate)
	))

(defmacro b (candidate) `(if (listp (car ,candidate))
	(mapcar 'second ,candidate)
	(second ,candidate)
	))

(defmacro i (candidate) `(if (listp (car ,candidate))
	(mapcar 'first ,candidate)
	(first ,candidate)
	))

(defmacro d (candidate) `(if (listp (car ,candidate))
	(mapcar 'first ,candidate)
	(first ,candidate)
	))
...
(defclass wildcardrule ()
	...
	(*-vars :initarg :*-vars
		:type list
		:accessor *-vars
		;This is a list with the pattern of variables, counting from 0. Ex.'(0 1 3) is a wildcard rule applied on variable 3 and thereafter.
		;It skips its neighbour candidate and checks the candidate together with the two following variables (0 and 1).
		)
	(backjump :initarg :backjump
		:type fixnum
		:accessor backjump
...
;;;;;;;;; due to preset backjumping (this info is not used in heuristic rules, ;;;;
...
(defun save-engine-state-end (filename searchindex searchspace-state searchspace solution n)
	;only the n last indexes will be saved (index will be reset, counting from the beginning of the susequence)
		  (declare (type array searchspace searchspace-state))

		  (let ((saved-index (if (< n (get-index searchindex)) n   (get-index searchindex))  ))

		  (write-engine-state-to-file filename (list (list 'index saved-index)
		  											 (list 'state  (last (2d-array-to-list searchspace-state) saved-index))
		  											 (list 'space (last (2d-array-to-list searchspace) saved-index))
		  											 (list 'solution (last (2d-array-to-list solution) (1+ saved-index)))
		  											 ))))
...
(defun read-engine-state-file (filename)
	(if (probe-file filename)
	  	(with-open-file (stream filename) (read stream nil))
	 	nil ))


(defmacro set-engine-state (filename)
	`(let ((state-from-file (read-engine-state-file ,filename)))
		(if (not state-from-file ) (print "No state file found, searching instead for a new solution." )
			(progn
...
(defmacro set-engine-state-until-index (filename)
	;the searchspace and searchspacestate saved in the file will overwrite the once in the engine 
	;ONLY UNTIL THE INDEX 

	`(let ((state-from-file (read-engine-state-file ,filename))
					temp-searchspacestate temp-searchspace temp-solution)
			(if (not state-from-file ) (print "No state file found, searching instead for a new solution." )
				(progn
					(setf previous-index (cadr (find 'index state-from-file :key #'first)))
	  				...
	  				(setf temp-solution (list-to-2d-array (cadr (find 'solution state-from-file :key #'first))))
	  				(setf (get-index searchindex) previous-index)
	  				...
	  				(loop for n from 0 to previous-index 
	  					do (progn (loop for m from 0 to 1
					 		         do (setf (aref solution n m)  (aref temp-solution n m)))))
			)))
	)
...
(defun engine_v4.05 (no-variables domain var-type abs-interval? start rulelist heurrulelist mode rnd? read-write-engine-state?)
		;;;;;;;; setup searchspace
...
;;;;;;; load a state of the search engine from file
		...
		(when (= read-write-engine-state? 4)
 		...
 		(when (or (= read-write-engine-state? 2) (= read-write-engine-state? 5) (= read-write-engine-state? 6))
...
;;;;;;;
			...
			(when (or (= read-write-engine-state? 1) (= read-write-engine-state? 3) (= read-write-engine-state? 4) (= read-write-engine-state? 5))
				(save-engine-state "state.txt" searchindex  searchspace-state searchspace solution))

			(when (= read-write-engine-state? 6) (save-engine-state-end "state.txt" searchindex  searchspace-state searchspace solution 10))
...
;;;;;;;;;;;;;;;;;;;;;;;;;HERE CONTINUE
	...
	;when absolute domain, set first interval to 0 (this is teh default before two pithces exist)
	(when (and (= (get-index searchindex) 0) (= abs-interval? *absolute-domain*)) (write-one-interval-value-to-solution 0 0 solution))
									...
									(declare (type list i-vars-candidates))
										(setf i-vars-candidates (loop for i fixnum in  (get-varidx-for-any-rule (get-rule heurrulevector heurruleid) (get-index searchindex))
																	collect  (if (= i (get-index searchindex))
													        				    (list (if (= abs-interval? *basic-domain*)
													        				    	0
													        				    	(-  (candidate searchspace i candidate-idx) (aref solution i 1))) (candidate searchspace i candidate-idx))
																				(list (aref solution (1+ i) 0)(aref solution (1+ i) 1)))
													))						
										  ;;;;;;;THIS IS WHERE A RULE IS CHECKED
...
(defun backtrack (searchspace-state searchindex test-result analyze-backjump-function)
	;This backtrack function will not allow backjump to the first variable -
	;in case a backjump is proposed all the way to the beginning, intermediate variables might not be visited
...
(engine_v4.02 15  '(1/4 -1/8 -1/4) 'ratio *duration-domain*  nil
	(list 
...
(engine_v4.02 10 '(0 1/4 1/2 3/4 1 1/8 3/2) 'ratio *absolute-domain* 0
	(list 
	  ;(format-wildcardrule #'(lambda (x) (not (zerop (d x)))) '(0))
...
(engine_v4.02 15  '(60 62 64 65 67 69 71 72) 'integer *absolute-domain*  nil
	(list 
...
(engine_v4.02 15  '(1 2 -2 -1)  'integer *interval-domain*  60
	(list 
	  (format-wildcardrule #'(lambda (x y)  (=   (print (i (print x))) (i y))) '(0 2))
...
(engine_v4.02 15  '('green 'blue 'red)  't *basic-domain*   nil
	(list 
	  ;(format-wildcardrule #'(lambda (x y)  (equal  (print x) (print y))) '(0 2))
	  (format-indexrule #'(lambda (x) (print (a x))) '(2))
	)
	nil 2 t 1
)


;no-variables domain var-type abs-interval? start rulelist heurrulelist mode rnd? read-write-engine-state?
;Example with pitches
(time (engine_v4.04 10001  '(60 62 64 65 67 69 71 72) 'integer *absolute-domain*  nil
	(list 
	  (format-wildcardrule #'(lambda (x) (<= (abs (i (print x))) 2)) '(0))
	  (format-indexrule #'(lambda (x) (= (a x) 62)) '(0))
	)
	nil 3 t 1
))
