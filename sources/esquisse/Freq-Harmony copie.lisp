...
to the following formula: carr. ± (i * mod.), i = 0,1,...<index>
...
(if (equalp unit "freq") (freq-mod/f carrier modul index output)
...
and filled with permutations of <notes> and intervals among <intervals>."
  ...
  (let (note note-oct)
    (let ((serie (list! serie)) (notes (list! notes)) (ints (list! ints)))
      (if (endp ints)
          (list (prog1 serie (format ccl::*so* "~A~A~%" (midi->notename (reverse serie)) (reverse serie))))
        (mapcan
...
#|
(PWGLDef TXtun1 ((tune fix (:value 0 :min-val -64  :max-val 63))
                (canal approx)) list 
"accord TX: 0= bécarre  21= +1/8  42= +1/4  63= + 3/8"
  (pw::midi-o (list 240 67 (+ 15 canal) 4 64 (+ 64 tune) 247)))

(PWGLDef TXtune ((tunings fix/fl/list) (chans fix/fl/list (:value 1))) list 
