...
; defclass fakes
(defclass bpf () (def-slot def))  
(defclass bpf-lib () (def-slot def))  
(defclass bpc () (def-slot def))  
...
(defmethod linear-fun ((x0 number) (y0 number) (x1 number) (y1 number)) 
  "Constructs a linear function passing through the points (x0 y0) and (x1 y1).
...
(defmethod interpole-points ((v1 t) (v2 t) (nbsteps integer) &optional profil)
 "Interpolates <nbsteps> values between <v1> and <v2> following a profile.
...
"Reduces <points> by removing all points closer than [<approx> * the amplitude range of the function] to the corresponding interpolated values.
...
(defmethod reduce-n-points (points n &optional (precision 10))
 "Reduces <points> to less than <n> points using approximations with the function REDUCE-POINTS.
...
(defmethod bpf-interpol ((first bpf) (second bpf) (steps number) &optional (curve 0.0) (decimals nil) (mode 'points))
          "Interpolates between two BPFs or two BPCs (<first> and <second>).
...
(defmethod bpf-offset ((self bpf) offset)
            "Generates a new BPF by addif <offset> to the x-points of <self>"
...
(defmethod bpf-crossfade ((bpf1 bpf) (bpf2 bpf) &key xfade-profile)
           "Generates a new BPF by crossfading the overlapping interval between BPF1 and BPF2.
...
(defmethod bpf-extract ((self bpf) x1 x2) 
 "Extracts a segment (between <x1> and <x2>) from <self>."
...
(defmethod bpf-scale ((self bpf) &key x1 x2 y1 y2)
  "Rescales <self> betwenn the supplied X (<x1>,<x2>) and/or Y (<y1>,<y2>) values."
...
; (save-lisp-and-die "/Volumes/Data/Desktop/pw+fun+iter.core")
