...
;;
;; This exemple show how to train a mall recurrent Elman Multi-Layer Perceptron to detect a beat in a stream of data from an accelerometer and, then, to play with as a UDP server into some intranet to trig sound or control tempo of some sequence with some digital audio workstation as a UDP client, such as Puredata ((see http://www.puredata.info). To run this example, one can use Emacs, Slime and SBCL Lisp environment
;;
