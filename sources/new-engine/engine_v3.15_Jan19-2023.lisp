...
;There is a new argy=ument for the angine function: read-write-engine-state?. It has four modes
;
...
(defun save-engine-state (filename searchindex searchspace-state searchspace)
		  (declare (type array searchspace searchspace-state))
		  (print  (typep searchspace-state 'array))
...
(defun engine_v3.15 (no-variables domain var-type rulelist heurrulelist mode rnd? read-write-engine-state?)
		;;;;;;;; setup searchspace
...
;;;;;;;
			...
			(when (or (= read-write-engine-state? 1) (= read-write-engine-state? 3))
...
;;;;TEST FUNCTION
	...
	(engine_v3.15 12 '(4 9 10 8 7 1 12 6 11 2 5 3)  'integer
	(list 
